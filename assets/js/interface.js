(function($) {
    ...
    $.fn.initCountdown = function() {
        $(this).each(function() {
            const _this = $(this);
            _this.countdown(_this.data('date'), {
                elapse: true
            }).on('update.countdown', function(event) {
                if (event.elapsed) {
    ...
    $.fn.initStickyMenu = function() {
        ...
        function init() {
            ...
            ck_sticky = false;
            $header.addClass('sticky_prepared').css({
                height: headerHeight
            });
    ...
    $.fn.kallesLoadQuikShop = function() {
        ...
        if (data.length) {
            $.magnificPopup.open({
                items: {
                    src: '<div class="mfp-with-anim pp_qs" id="content_quickview">' + data + '</div>',
                    /* can be a HTML string, jQuery_T4NT object, or CSS selector*/
                    type: 'inline'
                },
                tClose: 'Close (Esc)',
                removalDelay: 500,
